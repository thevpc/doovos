 ...
 * Time: 23:08:51
 */
public class CdCmd implements DShellInternalCmd {
    public void exec(String[] args, DShell shell) throws Exception {
        if (args.length == 1) {
            File f = new File(args[0]);
            if (f.exists() && f.isDirectory()) {
                DProcess.getProcess().setCwd(f.getCanonicalPath());
            } else {
                throw new CmdSyntaxError(args,getName(),getSyntax(),"Unable to change to " + f.getCanonicalPath());
            }
        } else {
