 ...
 */
public class JJIT_AASTORE_IINC extends JJITAbstractInstruction implements Cloneable {
    private KMemoryManager c_memman;
    private JJITInstruction c_next;
    private int local;
    private int inc;

    public JJIT_AASTORE_IINC(int local, int inc) {
        this.local = local;
        this.inc = inc;
    }

    public JJITInstruction run(KFrame frame) throws Exception {
        KReference m_d = frame.popRef();
        int index = frame.popInt();
        c_memman.setReferenceArray(frame.popRef(), index, m_d);
