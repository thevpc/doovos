...
public final class JJIT_FNEG extends JJITInstructionDef {
    public static final JJIT_FNEG INSTANCE = new JJIT_FNEG();

    private JJIT_FNEG() {
        super(KOperator.FNEG);
    }

    @Override
    public JJITChanges getChanges(JJITChangeModel model) {
        return new JJITChanges(JJITChangeOp.POP_FLOAT, JJITChangeOp.PUSH_FLOAT);
    }

    public void visitImpl(JJITClassSource cls, JJITFlowControlNode node, int relativeIndex) {
        final JJITMethodSourceRun r = cls.run();
        r.push(new JJITNew(Types.KFLOAT, r.popFloat().neg()));
    }
    ...
    public JJITInstruction createSingleInstruction(Object[] constantDefs, IntHashMap labels) {
        return new JJITInstructionSingle() {
            ...
            public JJITInstruction run(KFrame frame) throws Exception {
                frame.push(new KFloat(-frame.popFloat()));
                return next;
            }
        };
    }
