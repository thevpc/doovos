...
*/
class NativeParamStringArray implements NativeParam {
    public static final NativeParam INSTANCE = new NativeParamStringArray();
    public Object toHost(KRegister reg, KFrame frame) throws RemoteException {
        KReference ref = (KReference) reg;
        if (!ref.isNull()) {
            return new StringArrayAdapter(frame.getProcess(), ref).toJavaArray();
        }
        return null;
    }
