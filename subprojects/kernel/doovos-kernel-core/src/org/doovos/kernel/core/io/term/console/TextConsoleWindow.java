 ...
 * @see enigma.console.TextWindow
 */
class TextConsoleWindow extends JComponent implements TextWindow, Scrollable {
    ...
    private Timer repaintTimer = new Timer(REPAINT_DELAY, new ActionListener() {
        public void actionPerformed(ActionEvent e) {
            synchronized (TextConsoleWindow.this) {
                if (!valid || snapToBottom) {
                    JScrollPane sp = (JScrollPane) SwingUtilities.getAncestorOfClass(JScrollPane.class, TextConsoleWindow.this);
                    if (!valid) {
                        invalidate();
                        if (sp != null)
                            sp.validate();
    ...
    /**
     * Constructs a new <code>Java2DTextWindow</code> of the specified memorySize.
     ...
     * component's memorySize will result in these values being recomputed.
     * <p/>
     ...
     */
    public TextConsoleWindow(int columns, int rows, int scrollback) {
    ...
    private synchronized void setGridSize(int columns, int rows, int logicalColumns) {
        while (cursorY >= rows) scrollByOneRow();
    ...
    protected boolean processKeyBinding(KeyStroke ks, KeyEvent e, int condition, boolean pressed) {
        ...
        if (e.getKeyCode() == KeyEvent.VK_UP || e.getKeyCode() == KeyEvent.VK_DOWN ||
                e.getKeyCode() == KeyEvent.VK_PAGE_UP || e.getKeyCode() == KeyEvent.VK_PAGE_DOWN || e.getKeyCode() == KeyEvent.VK_TAB)
            return true;
    ...
    protected final void advance(char c) {
        ...
        switch (c) {
            case '\n':
                cursorX = 0;
                if (++cursorY >= rows) scrollByOneRow();
                break;
            case '\r':
                break;
            default:
    ...
    private void outputRaw(char c, TextAttributes attr, boolean immediate) {
        ...
        if (c == '\t') {
            ...
            try {
                ...
                if (startingCursorX >= 0 && startingCursorY >= 0) {
    ...
    public void clearWindow() {
        /** Number of rows currently being remembered. */
        setGridSize(columns, 0, logicalColumns);
//        rows=0;
//
//        /** Number of columns in each row. */
//        columns=0;
//
//        chars=new char[0][];                // [rows][columns]
//        attributes=new TextAttributes[0][]; // [rows][columns]
//
//        cursorX = -1;
//        cursorY = -1;
//        r
