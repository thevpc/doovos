 ...
 */
public final class KJVM_DRETURN extends KInstructionIdx implements Cloneable, KInstructionReturn {
    public static final KJVM_DRETURN INSTANCE = new KJVM_DRETURN();

    private KJVM_DRETURN() {
        super(KOperatorCodes.DRETURN);
    }

    public int run(KFrame context) throws RemoteException {
        KFrame c = context;
        KRegister value = c.pop();
        context = context.getThread().popFrame();
        context.push(value);
