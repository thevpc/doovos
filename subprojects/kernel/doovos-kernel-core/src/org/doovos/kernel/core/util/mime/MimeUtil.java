...
public class MimeUtil {
    ...
    public static String getMimeType(byte[] buffer) {
        String mimeType = null;
        try {
            mimeType = MimeUtil.getMagicMimeType(buffer);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            if (mimeType == null) {
    ...
    static {
        ...
        try {
            ...
            ((Properties) mimeTypes).load(MimeUtil.class.getResourceAsStream("mime-types.properties"));
        ...
        try {
            ...
            if (!found) {
                ...
                is = MimeUtil.class.getResourceAsStream("magic.mime");
            }
            parse(new InputStreamReader(is));
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println(Long.parseLong("00000200", 16));
        File dir = new File(".");
...
//		System.out.println(MimeUtil.getMinorComponent(null));
     ...
     * @author Taha BEN SALAH
     */
    private static String getMagicMimeType(byte[] buffer) throws IOException {
        int len = mMagicMimeEntries.size();
        for (int i = 0; i < len; i++) {
            MagicMimeEntry me = (MagicMimeEntry) mMagicMimeEntries.get(i);
            String mtype = me.getMatch(buffer);
            if (mtype != null) {
                return mtype;
            }
        }
